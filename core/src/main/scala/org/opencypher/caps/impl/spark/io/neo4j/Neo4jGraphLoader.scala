...
import org.opencypher.caps.api.global.{GlobalsRegistry, PropertyKey}
import org.opencypher.caps.api.record.{RecordSlotColumnName, _}
import org.opencypher.caps.api.schema.{Schema, VerifiedSchema}
import org.opencypher.caps.api.spark.{CAPSGraph, CAPSRecords, CAPSRecordsTokens, CAPSSession}
    ...
    new CAPSGraph {
      ...
      private def computeNodeFields(name: String, cypherType: CTNode)
        ...
        val labelFields = labels.map { name =>
          ...
          val field = StructField(RecordSlotColumnName.of(slot), BooleanType, nullable = false)
          slot -> field
        }
        val propertyFields = labels.flatMap { l =>
          schema.nodeKeys(l).map {
            case (key, t) =>
              ...
              val field = StructField(RecordSlotColumnName.of(slot), toSparkType(t), nullable = true)
              slot -> field
          }
        }
        val nodeSlot = OpaqueField(node)
        val nodeField = StructField(RecordSlotColumnName.of(nodeSlot), LongType, nullable = false)
      ...
      private def computeRelFields(name: String, cypherType: CTRelationship)
        ...
        val typeField = StructField(RecordSlotColumnName.of(typeSlot), IntegerType, nullable = false)

        val idSlot = OpaqueField(rel)
        val idField = StructField(RecordSlotColumnName.of(idSlot), LongType, nullable = false)

        val sourceSlot = ProjectedExpr(StartNode(rel)(CTNode))
        val sourceField = StructField(RecordSlotColumnName.of(sourceSlot), LongType, nullable = false)
        val targetSlot = ProjectedExpr(EndNode(rel)(CTNode))
        val targetField = StructField(RecordSlotColumnName.of(targetSlot), LongType, nullable = false)
