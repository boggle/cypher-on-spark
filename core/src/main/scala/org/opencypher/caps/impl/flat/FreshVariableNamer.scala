...
import org.opencypher.caps.api.record.{RecordHeader, RecordSlotColumnName}
...
object FreshVariableNamer {
  ...
  def generateUniqueName(header: RecordHeader): String = {
    ...
    val name = RecordSlotColumnName.from(Some(String.valueOf(chars.toArray)))

    if (header.slots.map(RecordSlotColumnName.of).contains(name)) generateUniqueName(header)
    else name
  }
