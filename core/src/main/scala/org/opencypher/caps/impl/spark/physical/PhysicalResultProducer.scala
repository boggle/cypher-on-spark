...
import org.opencypher.caps.api.global._
...
case class RuntimeContext(parameters: Map[ConstantRef, CypherValue], tokens: TokenRegistry, constants: ConstantRegistry) {
  def columnName(slot: RecordSlot): String = RecordSlotColumnName.of(slot)
  def columnName(content: SlotContent): String = RecordSlotColumnName.of(content)
}
