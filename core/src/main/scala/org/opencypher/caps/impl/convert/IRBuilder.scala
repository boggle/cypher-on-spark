...
import org.opencypher.caps.api.expr.{Expr, _}
import org.opencypher.caps.api.global.GlobalsRegistry
import org.opencypher.caps.impl.CompilationStage
import org.opencypher.caps.ir.api.block._
import org.opencypher.caps.ir.api.{CypherQuery, IRField, QueryInfo, QueryModel}
...
object IRBuilder extends CompilationStage[ast.Statement, CypherQuery[Expr], IRBuilderContext] {
  ...
  private def convertClause[R: _mayFail : _hasContext](c: ast.Clause): Eff[R, Vector[BlockRef]] = {
    ...
    c match {
      ...
      case clause@ast.With(_, ast.ReturnItems(_, items), ast.GraphReturnItems(_, Seq()), orderBy, skip, limit, where) if !items.exists(_.expression.containsAggregate) =>
        for {
      ...
      case ast.With(_, ast.ReturnItems(_, items), ast.GraphReturnItems(_, Seq()), _, _, _, None) =>
        for {
          fieldExprs <- EffMonad[R].sequence(items.map(convertReturnItem[R]).toVector)
          context <- get[R, IRBuilderContext]
          refs <- {
  ...
  private def registerOrderAndSliceBlock[R: _mayFail : _hasContext](orderBy: Option[ast.OrderBy],
