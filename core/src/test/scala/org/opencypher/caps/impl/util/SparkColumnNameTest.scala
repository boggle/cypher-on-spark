...
import org.opencypher.caps.api.record.RecordSlotColumnName
import org.opencypher.caps.common.test.BaseTestSuite
...
class SparkColumnNameTest extends BaseTestSuite {
  ...
  def fromString(text: String) = RecordSlotColumnName.from(Some(text))
}
